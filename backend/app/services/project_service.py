...
async def create_project(conn: asyncpg.Connection, user_id: str, data: ProjectCreate) -> ProjectOut:
    uid = to_uuid(user_id)
    await _assert_category_owned(conn, data.category_id, uid)
    row = await conn.fetchrow(
        f"""
...
async def update_project(conn: asyncpg.Connection, project_id: UUID, user_id: str, data: ProjectUpdate, tz_str: str = "UTC") -> ProjectOut:
    ...
    if "category_id" in updates:
        await _assert_category_owned(conn, updates["category_id"], uid)
    if not updates:
        row = await conn.fetchrow(
...
_CATEGORY_COLS = "id, user_id, name, created_at, updated_at"


async def _assert_category_owned(conn: asyncpg.Connection, category_id, uid: UUID) -> None:
    """The FK only proves the category exists, not that it's this user's."""
    if category_id is None:
        return
    owned = await conn.fetchval(
        "SELECT 1 FROM project_category WHERE id = $1 AND user_id = $2",
...
async def list_categories(conn: asyncpg.Connection, user_id: str) -> list[ProjectCategoryOut]:
    uid = to_uuid(user_id)
    rows = await conn.fetch(
        """
        ...
        LEFT JOIN project p ON p.category_id = c.id AND p.user_id = c.user_id
        WHERE c.user_id = $1
        GROUP BY c.id
    ...
    return [ProjectCategoryOut(**dict(r)) for r in rows]


async def create_category(conn: asyncpg.Connection, user_id: str, data: ProjectCategoryCreate) -> ProjectCategoryOut:
    uid = to_uuid(user_id)
    ...
    try:
        row = await conn.fetchrow(
            f"INSERT INTO project_category (user_id, name) VALUES ($1, $2) RETURNING {_CATEGORY_COLS}",
            uid, name,
        )
    except asyncpg.UniqueViolationError:
        raise HTTPException(status_code=409, detail=f'Category "{name}" already exists')
    return ProjectCategoryOut(**dict(row), project_count=0)


async def update_category(conn: asyncpg.Connection, category_id: UUID, user_id: str, data: ProjectCategoryUpdate) -> ProjectCategoryOut:
    uid = to_uuid(user_id)
    name = data.name.strip()
    if not name:
        raise HTTPException(status_code=422, detail="Category name is required")
    try:
        row = await conn.fetchrow(
            f"""
            UPDATE project_category SET name = $3, updated_at = now()
            WHERE id = $1 AND user_id = $2
            RETURNING {_CATEGORY_COLS}
            """,
    ...
    count = await conn.fetchval(
        "SELECT COUNT(*)::int FROM project WHERE user_id = $1 AND category_id = $2",
        uid, category_id,
    )
    return ProjectCategoryOut(**dict(row), project_count=count)


async def delete_category(conn: asyncpg.Connection, category_id: UUID, user_id: str) -> dict:
    """Projects keep existing; the FK is ON DELETE SET NULL, so they just go uncategorized."""
    uid = to_uuid(user_id)
    deleted = await conn.fetchval(
        "DELETE FROM project_category WHERE id = $1 AND user_id = $2 RETURNING id",
        category_id, uid,
    )
    if not deleted:
        raise HTTPException(status_code=404, detail="Category not found")

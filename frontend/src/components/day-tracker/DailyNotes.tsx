...
export function DailyNotes({ selectedDate }: Props) {
  const { data, isLoading } = useDailyNotes(selectedDate);
  const updateNotes = useUpdateDailyNotes(selectedDate);
  ...
  // Sync draft when data loads or date changes
  useEffect(() => {
    if (data && !editing) {
  ...
  const handleSave = async () => {
    await updateNotes.mutateAsync(draft);
    setEditing(false);
  };
  ...
  return (
    <div className="mt-8">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Daily Notes</span>
        {!editing && (
          <button
            onClick={handleEdit}
